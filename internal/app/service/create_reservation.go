...
func (s *serviceImpl) CreateReservation(
	...
	if err = s.checkReservationTimeAllowed(ctx, tx,
		roomID,
		startTime,
		endTime,
	); err != nil {
	...
	//
	reservation, err := s.db.Rooms.InsertReservation(ctx, tx,
...
// checkReservationTimeAllowed...
func (s *serviceImpl) checkReservationTimeAllowed(
	ctx context.Context,
	...
	// check if reservation start is too far in the past
	...
	dbReservations, err := s.db.Rooms.GetDBReservations(ctx, tx,
		-1, // ignore id
		-1, // ignore user
		roomID,
		checkTimeStart,
	...
	if err != nil {
		logrus.Errorf("reading room[ID:%d] reservations failed: %v", roomID, err)
		return ErrDBError
	}
